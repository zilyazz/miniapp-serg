 ...
 *                     type: integer
 *                     nullable: true
 *                     description: Цена консультации в Telegram Stars. Если null, оплата Stars недоступна.
 *                   input_schema:
 *                     type: object
 *                   sort_order:
 *                     type: integer
...
/**
 * @openapi
 * /service/orders/stars/invoice:
 *   post:
 *     summary: Создать Telegram Stars invoice для покупки консультации
 *     description: |
 *       Создаёт заказ консультации со статусом `pending_payment`, создаёт запись в `payments`,
 *       затем вызывает Vultr relay: `TELEGRAM_RELAY_BASE_URL/telegram/create-invoice-link`.
 *
 *       Selectel backend не ходит напрямую в `api.telegram.org` для Stars invoice.
 *       Telegram webhook приходит на Vultr, а Vultr пересылает его на Selectel `/webhook`.
 *       После успешного webhook заказ перейдёт в статус `new` и станет виден админ-ботам.
 *
 *       Доступно только для Telegram-пользователей. Для VK используйте YooKassa/VK-методы.
 *
 *       Для Telegram-пользователей перед созданием оплаты проверяется `users.username`.
 *       Если username не сохранён, сервер вернёт `409 telegram_username_required`.
 *       В этом случае фронту нужно показать пользователю сообщение:
 ...
 *     responses:
 *       200:
 *         description: Ссылка на оплату Stars создана
 *         content:
 *           application/json:
 *             schema:
 *               type: object
 ...
 *               payment_id: "svc_stars_1_1711791200000_ab12cd34ef56"
 *               order_id: 42
 *       400:
 ...
 *                 value:
 *                   error: "service_stars_price_not_set"
 *               telegram_stars_only:
 *                 value:
 *                   error: "telegram_stars_only"
 *       409:
 *         description: Telegram-пользователь не может купить консультацию без username
 *         content:
 *           application/json:
 ...
 *     description: |
 *       Служебный webhook для Telegram-оплаты. Внешний фронт его не вызывает.
 *       В production Telegram webhook зарегистрирован на Vultr relay, а relay пересылает update сюда.
 *       Поддерживает успешную оплату кристаллов и консультаций (`payments.product_table = service_orders`).
 *     tags: [Payments]
 *     responses:
 *       200:
